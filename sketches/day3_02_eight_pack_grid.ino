/*
 * DAY 3 - Sketch 2: The 8-pack SoC wall
 * Goal: read all 8 INA219 sensors and show every pack's SoC on a 2x4 grid.
 * Each cell: pack number, SoC %, and a colour bar (green/yellow/red).
 * A pack whose sensor is missing shows "--".
 *
 * Wiring: every INA219 shares the same I2C bus (SDA=21, SCL=22, VCC=3V3, GND=GND).
 * Each sensor must have a DIFFERENT address (see README_Setup.md).
 */
...
// One sensor object per pack, in address order (see README table).
...
// Grid layout on the 96x64 panel
#define GRID_COLS 2
#define GRID_ROWS 4
#define HEADER_H  11
...
void drawCell(uint8_t i, int soc) {     // soc = -1 means "no sensor"
  uint8_t col = i % GRID_COLS;
  uint8_t row = i / GRID_COLS;
  int x = col * CELL_W;
  int y = HEADER_H + row * CELL_H;
...
void setup() {
  ...
  for (uint8_t i = 0; i < NUM_PACKS; i++) {
    ...
    Serial.print("Pack "); Serial.print(i + 1);
    Serial.println(present[i] ? ": OK" : ": MISSING");
  }
}
...
void loop() {
  ...
  display.print("Cell SoC");

  for (uint8_t i = 0; i < NUM_PACKS; i++) {
    if (present[i]) {
      float v = ina[i].getBusVoltage_V();
      drawCell(i, socFromVoltage(v));
    } else {
      drawCell(i, -1);
    }
  }
